...
function AppointmentsManage() {
  ...
  // const [calendarMode, setCalendarMode] = useState("month"); // month/week - for future toggle
  // For demo: filter by pet
  const [calendarPetId, setCalendarPetId] = useState("");

  /** Utility: Format date/time strings for UI */
  ...
  // --- Handler: Open/close modals, start booking wizard ---
  ...
  function handleBookingStepNext() {
    ...
    if (current.valid) {
      if (bookingStep === BOOKING_STEPS.length - 1) {
        // Final step: submit (mock create) appointment
        const newAppt = {
  ...
  // --- Modals for appointment actions (view/cancel/details) ---
  ...
  // --- Reminders logic (demo mark as done) ---
  ...
  // --- Document Attachment logic for appointments ---
  ...
  // 1. Upcoming Appointments Section (with action modals)
  ...
  // 2. Calendar Section (month/week view, pet filter)
  function renderCalendar() {
    // Filtered appointments by petId
    const filtered = calendarPetId
      ? upcoming.filter((a) => a.petId === parseInt(calendarPetId))
      : upcoming;
    // For demo, just a table of the current month's days with dots for appts
    const now = new Date();
  ...
  // 3. History Section (expandable, rebook)
  ...
  // 4. Reminders & Notification Side Column
...
// --- Document Attachment Preview for Appointments ---
...
// Pastel color assignment by type
...
// --- Helper functions needed in modals (non-react) ---
